...
整个作品最多 3 页带 3D 模型的页面，封面和结尾如有模型也计入上限。
先询问我的创作偏好，完成一屏样板并截图给我确认。
样板确认后，有可用的 subagent 工具时，优先并行制作独立章节，
由主 Agent 统一整合与验收；工具不可用时说明并自行完成。
```

“最多 3 页”是这个示例的成本控制条件，不是 Skill 的固定限制；可按需要修改。
