...
主题风格、章节分组和重点、照片同屏与转场比例均由你看过素材后按推荐方案决定；只做桌面 1280x800。先完成一屏并实际截图自查，通过后自主扩展，不等待我确认。已授权事项不重复询问。
```
...
上面两种示例仍会先检查样板。如果希望跳过样板、尽早并行制作，可在初始问答选择“直接全量并行（不做样板）”，或明确说：

```text
不做样板。共享基础和接口就绪后，在可用并发与预算范围内
让各页面尽量同时开工，自行逐页检查，整合后直接交付。
```

这省去首章串行等待，但没有提前用一屏确认风格的检查点；不会省略任何页面的截图自查、实际世界坐标验证和整合验收。制作中需要调整时，直接指出具体位置，例如“这章照片大一点”“夜景到结尾保持暗色”或“保留这张照片，不必为它单独建模”。
...
- [intake.md](references/intake.md)：决定哪些偏好需要询问，以及先看样板、样板自验和直接全量并行三种方式如何推进。
